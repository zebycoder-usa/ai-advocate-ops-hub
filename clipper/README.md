# Content Clipper (Islamic channels + itech.cuts)
...
The same pipeline serves every channel. Pick the selection profile with
`--channel`:

| Channel | Flag | Rules |
|---|---|---|
| Sadiqa's channel, Raised, Last Third | `--channel islamic` (default) | never cut mid-ayah or mid-hadith, 30 to 90 s |
| itech.cuts | `--channel tech` | exact quotes only, no invented specs or prices, 15 to 60 s, always #itechcuts |

itech.cuts example, Tesla's official Cybercab launch video:

```
python clipper.py "https://www.youtube.com/watch?v=grdt05okn3Q" --channel tech --clips 5
```

For tech, only clip official company videos, press events, or creators who
allow reuse. Add the source link in the description of every post.
